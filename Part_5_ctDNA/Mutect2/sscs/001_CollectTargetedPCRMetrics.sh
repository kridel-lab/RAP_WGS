...
#ls */sscs/*.sscs.sorted.bam  > sscs_bam_files_for_mutect_all_samples.txt

samples=sscs_bam_files_for_mutect_all_samples.txt
names=($(cat $samples))
...
ln -s /cluster/projects/kridelgroup/RAP_ANALYSIS/ANALYSIS/ConsensusCruncher/consensus_output/consensus/$sample /cluster/projects/kridelgroup/RAP_ANALYSIS/ANALYSIS/ConsensusCruncher/Mutect2/processing/$name.sscs.bam
cd /cluster/projects/kridelgroup/RAP_ANALYSIS/ANALYSIS/ConsensusCruncher/Mutect2/processing
sample=$name.sscs.bam #linked bam file that we can use
...
gatk BedToIntervalList \
      ...
      -O /cluster/projects/kridelgroup/RAP_ANALYSIS/ANALYSIS/ConsensusCruncher/Mutect2/processing/${name}.sscs_amplicon.interval_list \
      -SD $sample

gatk BedToIntervalList \
      -I $targets_interval_list \
      -O /cluster/projects/kridelgroup/RAP_ANALYSIS/ANALYSIS/ConsensusCruncher/Mutect2/processing/${name}.sscs_targets.interval_list \
      -SD $sample

#save new interval lists as variables which will be used as input for final picard function
amps=/cluster/projects/kridelgroup/RAP_ANALYSIS/ANALYSIS/ConsensusCruncher/Mutect2/processing/${name}.sscs_amplicon.interval_list
ints=/cluster/projects/kridelgroup/RAP_ANALYSIS/ANALYSIS/ConsensusCruncher/Mutect2/processing/${name}.sscs_targets.interval_list

gatk CollectTargetedPcrMetrics \
       -I $sample \
       -O ${out_folder}/${name}.sscs.output_pcr_metrics.txt \
       -R $fasta_file \
       --PER_TARGET_COVERAGE ${out_folder}/${name}.sscs.per_target_coverage.txt \
