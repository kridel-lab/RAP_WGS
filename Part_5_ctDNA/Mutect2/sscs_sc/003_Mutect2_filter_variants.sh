...
gatk GetPileupSummaries \
  ...
  -L $ints \
  -O $out_folder/${tum}.sscs.sc_getpileupsummaries.table

gatk CalculateContamination \
  -I $out_folder/${tum}.sscs.sc_getpileupsummaries.table \
  -O $out_folder/${tum}.sscs.sc_calculatecontamination.table

gatk FilterMutectCalls -R $fasta_file \
  -V $out_folder/Mutect2_VCF_output/${tum}.sscs.sc.vcf.gz -O $out_folder/Mutect2_VCF_output/${tum}.sscs.sc.filtered.vcf.gz
