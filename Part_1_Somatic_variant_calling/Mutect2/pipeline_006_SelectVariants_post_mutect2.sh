...
gatk SelectVariants \
   ...
   -O ${selected}/${tum_name}.selected.vcf.gz \
   --exclude-filtered true \
   --select-type-to-exclude INDEL\
   --exclude-intervals Y \

#normalize file
input_vcf=${selected}/${tum_name}.selected.vcf.gz

#normalize variants, send to standard out and remove duplicates.
vt normalize ${input_vcf} -r $fasta -o ${selected}/${tum_name}.selected.normalized.vcf.gz
