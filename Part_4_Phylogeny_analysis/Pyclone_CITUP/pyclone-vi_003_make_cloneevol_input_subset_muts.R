...
get_patient_pyclone_plot = function(patient){
  ...
  pp <-plot.variant.clusters(dat,
    ...
    pdf(paste(patient,'subset_mutations_flow.pdf', sep="_"), width=10, height=5, useDingbats=FALSE, title='')
...
#      ignore.clusters=c(2),
    ...
    write.table(mean_cps, file=paste(date, patient, "subset_mutations", "_clonevol_output_for_mapscape.txt", sep=""), quote=F, row.names=F, sep="\t")
