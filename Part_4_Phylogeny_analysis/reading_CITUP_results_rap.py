...
adjacency = hdf.get('/trees/1/adjacency_list') #source and target A-->B, B--C, A-->D for example (edges)
clone = hdf.get('/trees/1/clone_freq') 

adjacency.to_csv("pat001_adjacency.csv", header=False)
clone.to_csv("pat001_clone.csv", header=False)
